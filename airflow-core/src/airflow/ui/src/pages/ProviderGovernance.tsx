...
const ProviderRow = ({
  ...
  readonly provider: ProviderRowData;
}) => {
  ...
  return (
    <Table.Row>
      ...
      <Table.Cell>{provider.openPrs}</Table.Cell>
      <Table.Cell>
...
const ProviderGovernance = () => {
    ...
    async function load() {
      try {
        const [summaryRes, providersRes] = await Promise.all([
          axios.get<Summary>("/ui/provider-governance/summary"),
          axios.get<{ providers: ProviderRowData[] }>("/ui/provider-governance/providers"),
        ]);
  ...
  const topBest = [...providers].sort((a, b) => b.healthScore - a.healthScore).slice(0, 2);
  const mostAtRisk = [...providers].sort((a, b) => a.healthScore - b.healthScore).slice(0, 2);

  if (loading && !summary && providers.length === 0) {
    return (
      <Box overflow="auto" px={{ base: 2, md: 4 }} py={4}>
        <Heading size="lg">Provider Health Overview</Heading>
        <Text mt={2}>Loading provider metrics…</Text>
      </Box>
    );
  }
      ...
      {summary && (
        <SimpleGrid columns={{ base: 1, md: 3 }} gap={4} mb={6}>
          ...
          <Box bg="bg.surface" borderRadius="xl" borderWidth={1} p={4}>
            ...
            <Box mt={3}>
              {topBest.map((p) => (
                <HStack key={p.id} justifyContent="space-between" mt={2}>
